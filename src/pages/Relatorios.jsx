...
export function RelatoriosPage() {
  ...
  const statusChartData = {
    labels: Object.keys(analytics.byStatus).map(s => STATUS_CONFIG[s]?.label || s),
    datasets: [{
  ...
  const handleExportCSV = () => {
    const headers = ['Item', 'Unidade', 'Quantidade', 'Valor Unit.', 'Valor Total', 'Status', 'Trimestre']
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Card className="p-4 text-center bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200">
          <p className="text-3xl font-bold text-emerald-700">{unidades.length}</p>
        ...
        <Card className="p-4 text-center bg-gradient-to-br from-amber-50 to-amber-100 border-amber-200">
          <p className="text-2xl font-bold text-amber-700">{formatCurrency(analytics.total)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-slate-900">Valor por Trimestre</h3>
          </div>
          <div className="h-64">
            <Bar data={trimestreChartData} options={{ ...chartOptions, indexAxis: 'x' }} />
          </div>
        </Card>
        ...
        <Card className="p-6 lg:col-span-2">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-slate-900">Valor por Unidade Gestora</h3>
          </div>
          <div className="h-80">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* By Status Table */}
        <Card className="p-6">
          <h3 className="font-semibold text-slate-900 mb-4">Resumo por Status</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left py-2 font-semibold text-slate-600">Status</th>
                  <th className="text-right py-2 font-semibold text-slate-600">Qtde</th>
                  <th className="text-right py-2 font-semibold text-slate-600">Valor</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(analytics.byStatus).map(([status, data]) => (
                  <tr key={status} className="border-b border-slate-100">
                    <td className="py-3">
                      <span className={`status-badge ${STATUS_CONFIG[status]?.color}`}>
                        {STATUS_CONFIG[status]?.icon} {STATUS_CONFIG[status]?.label || status}
                      </span>
                    </td>
                    <td className="py-3 text-right font-medium">{data.count}</td>
              ...
              <tfoot>
                <tr className="bg-slate-50">
                  ...
                  <td className="py-3 text-right font-bold">{demandas.length}</td>
        ...
        {/* By Unidade Table */}
        <Card className="p-6">
          <h3 className="font-semibold text-slate-900 mb-4">Resumo por Unidade</h3>
          <div className="overflow-x-auto max-h-80 overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-white">
                <tr className="border-b border-slate-200">
                  <th className="text-left py-2 font-semibold text-slate-600">Unidade</th>
                  <th className="text-right py-2 font-semibold text-slate-600">Demandas</th>
                  <th className="text-right py-2 font-semibold text-slate-600">Valor</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(analytics.byUnidade)
                  ...
                  .map(([unidade, data]) => (
                    <tr key={unidade} className="border-b border-slate-100">
                      <td className="py-3 font-medium text-slate-900">{unidade}</td>
