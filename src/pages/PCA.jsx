...
export function PCAPage() {
  ...
  const handlePublishPCA = async () => {
    if (
      !window.confirm(
        'Tem certeza que deseja Publicar o PCA? Esta ação o tornará público.'
      )
    )
      return;
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {['Q1', 'Q2', 'Q3', 'Q4'].map((q) => {
              ...
              const percentage =
                stats.total > 0 ? (valor / stats.total) * 100 : 0;
      ...
      <Card className="p-6">
        ...
        {isPublished && (
          <p className="text-sm text-emerald-600 mt-3 flex items-center gap-1">
            <CheckCircle className="w-4 h-4" /> Este PCA já foi publicado em{' '}
            {pca?.published_at
              ? new Date(pca.published_at).toLocaleDateString()
              : 'data não registrada'}
            .
          </p>
        )}
