...
const POSTGRES_ERROR_CODES = {
  ...
  '23505': 'Já existe um registro com estes dados. Por favor, verifique se não está duplicado.',
  '23503': 'Não é possível excluir este registro pois existem outros dados vinculados a ele.',
...
function getErrorType(code) {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') return ERROR_TYPES.NETWORK

  // Erros de permissão
  if (code === '42501' || code === 401 || code === 403) return ERROR_TYPES.PERMISSION
...
function getErrorMessage(error) {
  ...
  if (error.message) {
    ...
    if (msg.includes('fetch') || msg.includes('network') || msg.includes('conexão')) {
