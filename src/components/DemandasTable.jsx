...
import { Edit2, Trash2, Eye, MoreVertical, ChevronDown, ChevronUp } from 'lucide-react'
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-100">
            {sortedDemandas.map((demanda, index) => {
              ...
              const priority = PRIORITY_CONFIG[demanda.prioridade] || PRIORITY_CONFIG[3]
            ...
            <div key={demanda.id} className="p-4">
              ...
              {isExpanded && (
                <div className="mt-4 pt-4 border-t border-slate-100 animate-fade-in">
                  {demanda.descricao && (
                    <p className="text-sm text-slate-600 mb-3">{demanda.descricao}</p>
                  )}
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-slate-500">Quantidade:</span>
                      <span className="ml-2 font-medium">{demanda.quantidade}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Previsão:</span>
                      <span className="ml-2 font-medium">{formatDate(demanda.data_prevista)}</span>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <Button size="sm" variant="secondary" onClick={() => onEdit(demanda)}>
                      <Edit2 className="w-4 h-4" /> Editar
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => onDelete(demanda.id)}>
